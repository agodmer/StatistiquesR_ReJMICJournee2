...
```{r, echo=F}
iris3=iris2[order(iris2$Petal.Length),]
head(iris3)
...
```{r, echo=F}
age <- donnees$age
ggplot(donnees) + aes(x = age) + geom_histogram(aes(fill=age)) 
...
```{r, echo=F}
age <- donnees$age
ggplot(donnees) + aes(x = age) + geom_histogram(aes(fill=age)) + ggtitle("La variable age sous forme d’histogramme")
...
classe1 <- donnees[donnees$age>=20 & donnees$age<=40,]
classe2 <- donnees[donnees$age>40 & donnees$age<=55,]
classe3 <- donnees[donnees$age>55 & donnees$age<=80,]

c1 <- ggplot(classe1) + aes(x = classe1$age) + geom_histogram(aes(fill=classe1$age),size=2.5) + ggtitle("Histogramme des densité avec la classe [20,40]")
...
```{r, echo=F}
boxp <- ggplot(data = donnees, aes(x = factor(0), y = age)) + geom_boxplot() + xlab("") + scale_x_discrete(breaks = NULL)
...
```{r, echo=F}
individus <- c(2:101)
diag_bar <- ggplot(donnees) + aes(x = sexe, fill=sexe,color=sexe) + geom_bar() + ggtitle("Représentation de la variable sexe")
diag_bar
```
...
nuage_point1 <- ggplot(donnees) + aes(x = individus,y=Alcool) + geom_point(size=2.5, color="darkseagreen4")  + ggtitle("Représentation de la variable alcool")

nuage_point1


diag_bar1 <- ggplot(donnees) + aes(x=Alcool) + geom_bar() + ggtitle("Représentation de la variable alcool")
...
```{r, echo=F}
nuage_point2 <- ggplot(donnees) + aes(x = taille,y=poids) + geom_point(size=1.2, color="goldenrod")
nuage_point2
...
```{r, echo=F}
boxp1 <- ggplot(donnees, aes(x=ronfle, y=age, color=ronfle)) + geom_boxplot() + ggtitle("Boîtes à moustaches de la variable age en fonction de la variable ronfle")
boxp1
